...
Param(
    ...
    [string] $projDir = "C:\Users\rross\AppData\Roaming\Microsoft\UserSecrets\9d9bf388-edc0-4453-b296-809e1fa900d3\" #$(Get-Location),
...
if (-not (Test-Path (Join-Path $projDir 'secrets.json')))
{
...
if ($outputs)
{
    ...
    if (Test-Path $(Join-Path $projDir secrets.json)) {
        $settings = Get-Content -Encoding utf8 $(Join-Path $projDir secrets.json) | ConvertFrom-Json
    }
    else {
    ...
    $settings | ConvertTo-Json -depth 100 | Out-File -Encoding utf8 $(Join-Path $projDir secrets.json)
